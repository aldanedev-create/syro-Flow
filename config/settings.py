...
# Never run production with a predictable key. A stable local fallback keeps
# development and the Django test runner usable without weakening production.
configured_secret = env('SECRET_KEY', default=None)
if configured_secret:
    SECRET_KEY = configured_secret
elif DEBUG or 'test' in sys.argv or 'check' in sys.argv:
    SECRET_KEY = 'local-development-only-syro-flow-key-7f3b5d9e2a1c'
else:
    raise ImproperlyConfigured('SECRET_KEY must be set when DEBUG=False.')

# Parse ALLOWED_HOSTS and ensure Vercel deployments are covered without
...
if VERCEL_BLOB_TOKEN:
    ...
    # Local disk is fine for development. Vercel's /tmp directory is
    # ephemeral, so production must configure the object-storage variables.
