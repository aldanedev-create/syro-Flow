...
def validate_file_content(value):
    """Reject executable uploads and verify common image payloads."""
    filename = value.name.lower()
    content_type = (getattr(value, 'content_type', '') or '').lower()
    blocked_types = {
        'application/x-msdownload', 'application/x-dosexec',
        'application/x-sh', 'text/x-shellscript', 'application/javascript',
    }
    if content_type in blocked_types or filename.endswith(('.exe', '.dll', '.bat', '.cmd', '.ps1', '.sh')):
        raise ValidationError(_('Executable files are not allowed.'), code='executable_file')

    if os.path.splitext(filename)[1].lower() in {'.jpg', '.jpeg', '.png', '.gif', '.webp'}:
        try:
            image = Image.open(value)
            image.verify()
            value.seek(0)
        except (UnidentifiedImageError, OSError, ValueError):
            raise ValidationError(_('The uploaded image is invalid or corrupted.'), code='invalid_image')
