...
class DonationTransaction(models.Model):
    ...
    def save(self, *args, **kwargs):
        """Generate IDs and account for verified status transitions once."""
        ...
        if previous_status != self.status:
            ...
            if self.status == 'completed' and previous_status != 'completed':
                delta = self.amount
            elif previous_status == 'completed' and self.status in {'refunded', 'failed'}:
                delta = -self.amount
            if delta:
                with transaction.atomic():
                    DonationGoal.objects.filter(
                        is_active=True,
                        currency=self.currency,
                    ).update(raised_amount=F('raised_amount') + delta)
